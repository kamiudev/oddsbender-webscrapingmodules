...
def add_url_redis(url, rd_game, stream_name):
    ...
    cursor_d, get_game_urls = r.scan(match=f'basketball_url_{rd_game}*', count=10000)
    ...
    if len(get_game_urls) != 0:
        ...
        if url not in res_lst:
            r.set(f'basketball_url_{rd_game}_{datetime.datetime.now().timestamp()}', f'{tmp_dict}')
...
def read_url_redis(sportbookname):
    ...
    if sportbookname == 'ALL':
        cursor, get_rd_counter = r.scan(match=f'basketball_url_*', count=10000)
    if sportbookname != 'ALL':
        cursor, get_rd_counter = r.scan(match=f'basketball_url_{sportbookname}*', count=10000)
...
def text_filter(text_in, aligned_bet_name='', home_team='', away_team='', side=''):

    if 'spread' in str(aligned_bet_name.lower()) or 'alternative handicap live' in str(aligned_bet_name.lower()):
        if re.compile(r'[a-zA-Z]').findall(text_in):
            if home_team in away_team or away_team in home_team:
                if '+' in text_in:
                    text_out = f'Home Team {text_in[text_in.index("+"):]}' if side == 1 else f'Away Team {text_in[text_in.index("+"):]}'
                if '-' in text_in:
                    text_out = f'Home Team {text_in[text_in.index("-"):]}' if side == 1 else f'Away Team {text_in[text_in.index("-"):]}'
            else:
                text_out = text_in.replace(home_team, 'Home Team ').replace(away_team, 'Away Team ')

            return text_out.replace('  ', ' ')

        if not re.compile(r'[a-zA-Z]').findall(text_in):
            text_out = f'Home Team {text_in}' if side == 1 else f'Away Team {text_in}'
            return text_out.replace('  ', ' ')

    if any(it in aligned_bet_name.lower() for it in ['moneyline', 'quarter winner', 'money']):
        if not re.compile(r'[a-zA-Z]').findall(text_in):
            text_out = f'Home Team' if side == 1 else f'Away Team'
            return text_out

    if any(it in text_in.upper() for it in ['HOME TEAM O', 'AWAY TEAM O', 'HOME TEAM U', 'AWAY TEAM U']):
        text_out = text_in.upper().replace('HOME TEAM ', '').replace('AWAY TEAM ', '')
        return text_out.replace('  ', ' ')
