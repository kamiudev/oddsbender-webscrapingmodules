...
def main():
    ...
    while datetime.now() < module_operate_until:
        ...
        try:
            ...
            get_all_sport = WebDriverWait(driver, 20).until(EC.visibility_of_all_elements_located((By.CSS_SELECTOR, '.v-tab')))
            games_present = 0

            for game in get_all_sport:
                if 'Football' in str(game.text):
                    game.click()
            ...
            if games_present == 1:
                active_page = WebDriverWait(driver, 5).until(EC.visibility_of_element_located((By.XPATH, "//div[@class='v-window-item active-tab']")))
                match_rows = active_page.find_elements(By.XPATH, ".//div[contains(@class, 'basic-event-row')]")
                for match_row in match_rows:
                    participants_card = match_row.find_element(By.XPATH, ".//a")
                    game_url = participants_card.get_attribute("href")
                    urls_list.append(game_url)
        ...
        if count_scraps % scrap_step == 0:
            actions_on_page(driver=driver, class_name="v-window-item active-tab")
